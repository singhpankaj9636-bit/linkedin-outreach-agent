...
- Cold email: subject line on first line, then blank line, then 4-5 SHORT lines with whitespace between each
...
CTA VARIATION:"""
            ...
            score_prompt = f"""Rate this outreach message on 3 metrics. Be strict and realistic.

Prospect: {prospect_name}, {job_title} at {company}
Pain point: {pain_point}
Recent activity: {recent_activity if recent_activity else 'None'}
Offer: {offer}

Give scores out of 10 for:
1. Personalization Score (how specific and tailored is the message)
2. Reply Probability (likelihood of getting a response)
3. Spam Risk (10 = very spammy, 1 = not spammy at all)

Also give one line of improvement tip.
...
TIP: [one line tip]"""
            ...
            for line in lines:
                if "PERSONALIZATION:" in line:
                    val = line.split(":")[1].strip().replace("/10","")
                    try:
                    ...
                    val = line.split(":")[1].strip().replace("/10","")
                    try:
                        st.metric("📬 Reply Probability", f"{val}/10")
                        st.progress(int(val)/10)
                    except: pass

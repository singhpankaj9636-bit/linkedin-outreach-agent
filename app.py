...
- NEVER use: "research suggests", "AI-powered solutions", "optimize outreach", "leverage automation", "hope you're doing well"
...
CTA VARIATION:"""
            ...
            # Bug 4 Fix — only st.code, no duplicate
...
TIP: one line improvement"""
            ...
            for line in lines:
                if "PERSONALIZATION:" in line:
                    try:
                        # Bug 2 Fix — float instead of int
            ...
            # Bug 5 Fix — save only on button click
...
# Bug 3 Warning
st.caption("⚠️ CRM data resets on app refresh. Export CSV to save permanently.")

if st.session_state.crm_data:
    st.write(f"**{len(st.session_state.crm_data)} leads saved**")
    ...
    for lead in st.session_state.crm_data:
        with st.expander(f"👤 {lead['Name']} — {lead['Company']}"):
            ...
            st.write(f"**Pain Point:** {lead['Pain Point']}")
